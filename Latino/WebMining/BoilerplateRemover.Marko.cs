...
namespace Latino.TextMining
{
    public class BoilerplateRemover
    {
        ...
        public enum FeaturesEnum
        {
             ...
             isInP = 4096,
             isInH = 8192,
             tbtFreq = 16384, // Text-Block-Tree frequency
             all = Int32.MaxValue
        }
        ...
        public static class FeatureNames
        {
            ...
            public static string tbtFreq = "tbtFreq";
            ...
            {                        
                ...
                if((fe & FeaturesEnum.tbtFreq) != 0)
                    features.Add(prec + tbtFreq);
...
//            public List<string> tokens = new List<string> ();
            ...
            public string ToString (FeaturesEnum fe)
            {
                ...
                if((fe & FeaturesEnum.isInH) != 0)
                    sb.Append(isInH ? "1," : "0,");
                if ((fe & FeaturesEnum.tbtFreq) != 0)
                    sb.Append(Math.Round (tbtFreq, 4));
            ...
            public void GetFeatures(Features feats, string precedence)
            {
                ...
                feats.Add(precedence + FeatureNames.tbtFreq, new FeatureValue(tbtFreq));
            }
...
//                conf = currentNode.classDist[0] / conf * 100;                
        ...
        private static string [] tagRegexStrings = new string []
        {
            ...
            @"<(?<startMark>/?)span(?:(\s+?class=""x-nc-sel(?<blockClassID>[012345]?)"")|.*?(?<endMark>/?)\s*?)>",
        ...
        private BOW bowBoilerplate = new BOW("Boilerplate");
        private BOW bowHeadline = new BOW("Headline");
        private BOW bowSupplement = new BOW("Supplement");
        private BOW bowFullText = new BOW("FullText");
        private BOW bowComments = new BOW("Comments");
        private BOW bowBlockTextFreqs = new BOW("BlockTextFreqs");
        
        private bool ignoreDecorativeTags = false;
            
        public BoilerplateRemover(bool ignoreDecorativeTags)
        {
            this.ignoreDecorativeTags = ignoreDecorativeTags;
        ...
        public BoilerplateRemover(string bowBoilerplateFile,
            string bowHeadlinesFile,
            string bowSupplementFile,
            string bowFullTextFile,
            string bowCommentsFile,
            string bowBlockTextFreqsFile)
        {
            if(String.IsNullOrEmpty(bowBoilerplateFile) == false)
                bowBoilerplate.Deserialize(bowBoilerplateFile);
            if(String.IsNullOrEmpty(bowHeadlinesFile) == false)
                bowHeadline.Deserialize(bowHeadlinesFile);
            if(String.IsNullOrEmpty(bowSupplementFile) == false)
                bowSupplement.Deserialize(bowSupplementFile);
            if(String.IsNullOrEmpty(bowFullTextFile) == false)
                bowFullText.Deserialize(bowFullTextFile);
            if(String.IsNullOrEmpty(bowCommentsFile) == false)
                bowComments.Deserialize(bowCommentsFile);
            if(String.IsNullOrEmpty(bowBlockTextFreqsFile) == false)
                bowBlockTextFreqs.Deserialize(bowBlockTextFreqsFile);
        ...
        public List<HtmlBlock> GetHtmlBlocks(string fileName, bool isAnnotated)
        {
            ...
            Stack<TextClass> textClasses = new Stack<TextClass>(); // to handle nested span tags
            ...
            while(tokensEnum.MoveNext())
            {
                ...
                {                    
                    if(ignoreDecorativeTags && IsTagIgnorable(t))
                    {
                        currentBlock.tokens.Add(tokensEnum.CurrentTokenIdx);
                        currentBlock.tokens.Add(tokensEnum.CurrentTokenIdx + tokensEnum.CurrentTokenLen);
                        continue;
                    ...
                    else // not anchor
                    {
                        ...
                        if(isAnnotated)
                        {
                            spanType = GetSpanTagType(t, out textClass);                            
                            if( (spanType != SpanTagType.NotSpanTag) && (spanType != SpanTagType.OpeningAndClosingSpanTag) )
                            {
                                bool closingUnknownSpan = false;
                                if( (spanType == SpanTagType.ClosingSpanTag) && (textClasses.Count > 1) )
                                {                                    
                                    if(textClasses.Peek() == TextClass.Unknown)
                                        closingUnknownSpan = true;                                    
                                    textClasses.Pop();
                                }
                                else if (spanType != SpanTagType.ClosingSpanTag)                                
                                    textClasses.Push(textClass);

                                bool lastTextClassDifferent = false;
                                foreach(TextClass c in textClasses)
                                    if(c != TextClass.Unknown)
                                    {
                                        if(c != currentClass)
                                            lastTextClassDifferent = true;
                                        currentClass = c;
                                        break;
                                    }
                                currentBlock.tokens.Add(tokensEnum.CurrentTokenIdx + tokensEnum.CurrentTokenLen);
                                if( (lastTextClassDifferent == false) && (closingUnknownSpan == false) )
                                    continue;
                            }
                        }
                        // we 'encountered' a tag and want to save the last (current) block if it contains any words
